...
Selain itu, Bootstrap juga menyediakan berbagai komponen seperti navbar, card, button, table, accordion, carousel, dan progress bar yang sudah memiliki desain standar. Hal ini membuat proses pembuatan antarmuka menjadi lebih efisien dan mengurangi kebutuhan penulisan CSS secara manual.

Dalam pengembangan halaman bertema Ramadan, Bootstrap dimanfaatkan untuk membangun tampilan yang terstruktur dan menarik dengan memanfaatkan warna, komponen, serta tata letak yang sudah tersedia. Penggunaan class bawaan seperti container, row, card, accordion, dan carousel membantu menghasilkan tampilan yang modern tanpa perlu banyak konfigurasi tambahan.
...
![Screenshot halaman web Ramadan Mubarak](result.png)

# Penjelasan
Program ini dibuat menggunakan HTML dengan bantuan framework Bootstrap 5 untuk menghasilkan tampilan halaman web bertema Ramadan dengan konsep dark mode yang elegan. Seluruh desain memanfaatkan class bawaan Bootstrap seperti layout grid, warna, dan berbagai komponen, sehingga tidak memerlukan CSS tambahan.

Pada bagian `<head>`, terdapat metadata serta link ke Bootstrap CSS dan Bootstrap Icons melalui CDN. Hal ini memungkinkan penggunaan berbagai komponen siap pakai dan ikon secara langsung tanpa instalasi tambahan.

Di bagian `<body>`, pertama terdapat **navbar** yang menggunakan class `navbar`, `bg-success`, dan `sticky-top`. Navbar ini berisi logo bertuliskan "Ramadan Mubarak" dengan ikon bulan dan bintang, serta menu navigasi yang mengarah ke setiap section halaman (Beranda, Amalan, Jadwal, Doa, dan Tentang). Navbar bersifat responsif dengan adanya toggler untuk tampilan mobile.

Bagian **hero section** menampilkan judul besar "Marhaban Ya Ramadan" dengan badge "Ramadan 1447 H", deskripsi singkat, serta dua tombol aksi (Panduan Amalan dan Jadwal Imsakiyah). Di sisi kanan terdapat ikon bulan besar yang memberikan kesan visual menarik. Layout menggunakan sistem grid Bootstrap (`row` dan `col-lg`) agar responsif.

Section **Amalan Ramadan** berisi empat card yang menjelaskan amalan utama di bulan Ramadan yaitu Puasa, Tadarus Al-Quran, Sholat Tarawih, dan Sedekah & Zakat. Setiap card memiliki ikon dalam lingkaran, judul, dan deskripsi singkat. Card dibuat menggunakan class `card`, `shadow-lg`, dan `rounded-4` dengan tampilan dark theme.

Section **Jadwal Imsakiyah** menampilkan tabel jadwal sholat untuk wilayah Purwokerto selama 5 hari pertama Ramadan 1447 H. Tabel menggunakan class `table-dark`, `table-striped`, dan `table-hover` untuk tampilan yang menarik. Waktu Imsak dan Maghrib ditandai dengan badge berwarna untuk kemudahan pembacaan.

Section **Doa-Doa Ramadan** menampilkan empat card berisi doa-doa penting yaitu Doa Niat Puasa, Doa Buka Puasa, Doa Lailatul Qadar, dan Doa Kamilin. Setiap card menampilkan teks Arab, transliterasi latin, dan terjemahan dalam Bahasa Indonesia. Card menggunakan border dan header berwarna info (biru) sebagai pembeda visual.

Section **Keutamaan Ramadan** menggunakan komponen **accordion** Bootstrap untuk menampilkan informasi secara interaktif. Terdapat empat item accordion yaitu Pahala Berlipat Ganda, Pintu Surga Dibuka, Lailatul Qadar, dan Pengampunan Dosa. Pengguna dapat mengklik setiap item untuk melihat penjelasannya.

Section **Target Ramadan** menampilkan **progress bar** untuk memantau progres ibadah meliputi Khatam Al-Quran, Sholat Tarawih, Sedekah Harian, dan Puasa Penuh. Setiap progress bar memiliki warna berbeda dan animasi striped untuk tampilan yang dinamis.

Section **Carousel Motivasi** menggunakan komponen **carousel** Bootstrap untuk menampilkan kutipan-kutipan hadits dan ayat Al-Quran secara bergantian. Setiap slide memiliki warna background berbeda (hijau, biru, kuning) untuk variasi visual.

Terakhir, bagian **footer** berisi tulisan "Ramadan Mubarak", doa penutup, dan credit pembuat. Footer menggunakan background hijau (`bg-success`) yang konsisten dengan tema keseluruhan halaman.

Secara keseluruhan, program ini menunjukkan pemanfaatan berbagai komponen Bootstrap secara maksimal seperti navbar, card, table, accordion, progress bar, carousel, dan grid system. Dengan menggunakan dark mode (`bg-dark`) sebagai tema utama dan warna hijau-kuning sebagai aksen Islami, halaman ini menghasilkan tampilan website yang modern, responsif, dan menarik tanpa perlu menulis CSS tambahan.